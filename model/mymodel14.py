...
class NET(nn.Module):
    def __init__(self):
        ...
        self.conv6 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=1)
    ...
    def forward(self, x):
        ...
        y1 = self.res5(y2+y1)
        z4 = y1 + x
        z1 = self.conv4(torch.cat([y3,y2],dim=1))
        z2 = self.conv5(torch.cat([y2,y1],dim=1))
        z3 = self.conv6(torch.cat([y1,z4],dim=1))
        z1 = self.res10(z1)
        z2 = self.res9(z1+z2)
        z3 = self.res8(z2+z3)
        output = z3 + z4
