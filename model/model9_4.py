...
#相比于model9_3，同样修改了Norm部分，下采样增加了8倍,用dwconv采样
#参数量：1.21M
...
class block1(nn.Module):
    ...
    def forward(self,x):
        ...
        x1 = self.conv1(x)
        x1 = self.dwconv0(x1)
        x1 = self.sigmod(self.conv2(x1))
        x2 =x * x1 + x
        y0 = self.conv3(x2)
        y1 = self.gelu(self.dwconv1(y0))
        y0 = self.dwconv2(y0)
        y3 = torch.cat([y1,y0],dim=1)
        y3 = self.conv4(y3) + x
        return y3
class block0(nn.Module):
    def __init__(self,n_feats,conv=default_conv,dwconv=common.DepthWiseConv):
        ...
        self.conv3 = dwconv(n_feats, n_feats)

        self.conv_end = conv(n_feats*4,n_feats,1)
    ...
    def forward(self,x):
        ...
        b4 = F.adaptive_max_pool2d(b3, (x.size(2) // 8, x.size(3) // 8))  # pooling
        b4 = self.gelu(self.conv3(b4))
        ...
        b4 = F.interpolate(b4, (x.size(2), x.size(3)), mode='bilinear', align_corners=True)  # upsample

        m = self.conv_end(torch.cat([b1,b2,b3,b4],dim=1))
        return m
...
class block2(nn.Module):
    ...
    def forward(self,x):
        ...
        x0 = self.conv1(x)
        x1 = self.gelu(x0 + self.conv2(x0-x))
        return x1
...
class base_block1(nn.Module):
    def __init__(self,nfeats=64,dwconv=common.DepthWiseConv,conv=default_conv):
        ...
        # self.base_block1 = nn.Sequential(
        #     base_block0(nfeats, nfeats),
        #     Myattn(nfeats),
        #     basic_block(nfeats, nfeats)
        # )
        self.attn1 = esa.ESA(nfeats)
        # self.base_block2 = nn.Sequential(
        #     base_block0(nfeats, nfeats),
        #     Myattn(nfeats),
        #     basic_block(nfeats, nfeats)
        # )
...
class NET(nn.Module):
    def __init__(self, scale=2,nfeats=64, conv=common.default_conv):
        ...
        self.block0 = base_block1(nfeats=64)
        self.block1 = base_block1(nfeats=64)
        self.block2 = base_block1(nfeats=64)
        self.block3 = base_block1(nfeats=64)
